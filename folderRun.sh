...
# Start in separate "Fastq" files inside of project folder
# ********make sure logLookup.txt (or aliquotId.txt and csid.txt list) is included in top project folder********
# Usage: folderRun.sh

cwd=$(pwd) 
bash $HOME/scripts/Batch_QualAssessRawSeqs.sh -p $cwd -o $cwd
bash $HOME/scripts/initialProcessing.sh $cwd
python $HOME/scripts/extract_TRIMMED_SPAdes.IDBA_genomes_V2.py $cwd
mkdir ../Fasta
mv *.fna ../Fasta/
cp 16sSummary.tsv ..
